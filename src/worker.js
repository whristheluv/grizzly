...
function providerIds(value) {
  if (!/^\d+(?:,\d+)*$/.test(String(value ?? ''))) throw new Error('Invalid provider IDs');
  return String(value);
}
...
export class PurchaseCoordinator {
  ...
  async tick() {
    ...
    const providers = providerIds(env.PROVIDER_IDS);
    const state = await this.ctx.storage.get(KEY);
    if (state?.status === 'purchased') {
      if (!state.notified) {
        await discord(env, `✅ GrizzlySMS Apple / Turkey 번호 구매 완료 (제공업체 ${providers})\n번호: ${state.phone}\n활성화 ID: ${state.id}`);
    ...
    if (state?.status === 'attempted') {
      if (!state.alerted) {
        await discord(env, '⚠️ GrizzlySMS 번호 구매 결과를 확인할 수 없어 자동 구매를 중지했습니다. GrizzlySMS 활성화 목록을 확인해 주세요.');
        await this.ctx.storage.put(KEY, { ...state, alerted: true });
      }
      return;
    }

    const count = stock(await api(env, 'getPricesV3', { service: 'wx', country: '62' }), providers);
    console.log(`Selected provider stock: ${count}`);
    if (count === 0) return;
    ...
    const result = await api(env, 'getNumber', {
      service: 'wx', country: '62', providerIds: providers, maxPrice: limit,
    });
...
export default {
  async scheduled(_controller, env) {
    const stub = env.PURCHASE.getByName('apple-turkey-providers');
