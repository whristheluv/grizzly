# GrizzlySMS Apple Turkey provider 405

1분마다 재고를 확인해 GrizzlySMS 잔액으로 **번호 1개만** 구매하고 Discord 웹후크로 번호와 활성화 ID를 보냅니다. 카드 결제나 GrizzlySMS 잔액 충전은 하지 않습니다. `getNumber` 요청에는 `providerIds=405`, `maxPrice=MAX_PRICE`가 적용됩니다. 구매 기록은 Cloudflare Durable Object에 보관합니다.
...
- 재고 0개면 구매하지 않습니다. 재고가 있으면 가격 상한을 적용해 `getNumber`를 한 번 호출합니다. `NO_NUMBERS`면 다음 실행에서 다시 확인합니다.
